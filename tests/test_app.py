...
class AppFlowTest(unittest.TestCase):
    ...
    def test_public_landing_page_and_protected_login_are_separated(self):
        ...
        self.assertIn(b"Projektmanagement f\xc3\xbcr handlungsorientierten Unterricht", landing)
        self.assertIn(b'content="index,follow,max-image-preview:large', landing)
        self.assertIn(b'"applicationCategory":"EducationalApplication"', landing)
        self.assertIn(b'"@type":"Organization"', landing)
        self.assertIn(b'id="fragen"', landing)
        self.assertIn(b"Was ist ProjektKontor?", landing)
        self.assertIn(b"Muss ProjektKontor installiert werden?", landing)
        self.assertIn(b"mit mehreren Lehrkr\xc3\xa4ften nutzen?", landing)
        ...
        self.assertIn(b"https://projektkontor.org/impressum", sitemap)
        self.assertIn(b"https://projektkontor.org/datenschutz", sitemap)
        status, impressum, _ = self.client.request("GET", "/impressum")
        self.assertEqual(status, 200)
        self.assertIn(b'content="index,follow"', impressum)
        self.assertIn(b'<link rel="canonical" href="https://projektkontor.org/impressum">', impressum)
        self.assertIn(b"PRIMEAdvisory", impressum)
        self.assertIn(b"Jeroen L. Jochem", impressum)
        self.assertIn(b"Nieberdingstr. 41", impressum)
        self.assertIn(b"info@prime-advisory.de", impressum)
        self.assertIn(b"0201 8438179-0", impressum)
        self.assertIn(b"DE453188253", impressum)
        self.assertIn(b"Kleingewerbe ohne Vorsteuerabzugsberechtigung", impressum)
        self.assertNotIn(b"legal-placeholder", impressum)
        status, privacy_page, _ = self.client.request("GET", "/datenschutz")
        self.assertEqual(status, 200)
        self.assertIn(b'<link rel="canonical" href="https://projektkontor.org/datenschutz">', privacy_page)
        self.assertIn(b'content="index,follow"', privacy_page)
        self.assertIn(b"PRIMEAdvisory", privacy_page)
        self.assertIn(b"Cloudflare Turnstile", privacy_page)
        self.assertIn(b"Keine Werbe- oder Analyseprofile", privacy_page)
        self.assertIn(b"Verantwortung von Schule oder Schultr\xc3\xa4ger", privacy_page)
        self.assertIn(b"sp\xc3\xa4testens 30 Tage", privacy_page)
        self.assertIn(b"nach 15 Minuten", privacy_page)
        self.assertIn(b"nach zw\xc3\xb6lf Stunden", privacy_page)
        self.assertIn(b"Art. 6 Abs. 1 lit. f DSGVO", privacy_page)
        self.assertIn(b'href="/datenschutz"', landing)
        self.assertIn(b"Datenschutzerkl\xc3\xa4rung", login)
